...
class LeadEmpresa(Base):
    ...
    capital_social: Mapped[float | None] = mapped_column(
        Numeric(18, 2)
    )

    socios: Mapped[str | None]

    origem_match: Mapped[str | None] = mapped_column(
        String(20)
    )

    cnae_match_secundario: Mapped[str | None] = mapped_column(
        String(255)
    )
...
class HubspotEmpresa(Base):
    ...
    nome_empresa: Mapped[str | None] = mapped_column(
        String(255)
    )
